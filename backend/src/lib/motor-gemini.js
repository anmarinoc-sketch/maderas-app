...
/**
 * Motor de consultas a Gemini, reutilizable por varias apps.
 *
 * Antes esto vivia dentro de gemini.js, atado a la identificacion de maderas. Al
 * aparecer BioScan hacian falta las mismas 300 lineas de rotacion de modelos y
 * traduccion de errores, pero con OTRA clave: el nivel gratuito limita 20 peticiones
 * diarias por modelo y por proyecto, asi que dos apps con la misma clave se comen la
 * cuota la una a la otra. De ahi la fabrica: cada app crea su motor con su clave y
 * lleva su propia contabilidad de modelos agotados.
 */
...
function traducirError(error) {
  ...
  switch (status) {
    case 400:
      return new AppError(502, 'PETICION_RECHAZADA', 'Gemini rechazo la peticion.', mensajeOriginal);
    case 401:
    case 403:
...
/**
 * Crea un motor con su propia clave y su propia cadena de modelos.
 *
 * @param {object} opciones
 * @param {string} opciones.apiKey   clave de Gemini. Nunca sale de este proceso.
 * @param {string[]} opciones.modelos  cadena de modelos, del mas al menos capaz.
 * @param {string} opciones.nombre   solo para los logs.
 */
export function crearMotor({ apiKey, modelos, nombre }) {
  ...
  function marcarAgotado(modelo, error) {
    ...
    console.warn(
      `[cuota:${nombre}] ${modelo} agotado (${esDiario ? 'limite diario' : 'limite por minuto'}), ` +
  ...
  function disponibles() {
    ...
    return modelos.filter((m) => !(agotados.get(m) > ahora));
  }

  /** Estado de la cadena de modelos, para /health. */
  function estadoModelos() {
    const ahora = Date.now();
  ...
  /**
   * Lanza una consulta y devuelve el JSON ya parseado.
   *
   ...
   * @param {Array}  peticion.partes        partes del turno de usuario (texto e imagenes).
   * @param {string} peticion.instruccion   instruccion de sistema.
   * @param {object} peticion.esquema       responseSchema de salida estructurada.
   * @param {number} [peticion.temperatura]
   * @returns {Promise<{ resultado: object, modelo: string, uso: object|null }>}
   */
  async function generar({ partes, instruccion, esquema, temperatura = 0.2 }) {
    const cadena = disponibles();
    if (cadena.length === 0) {
      const espera = Math.min(...modelos.map((m) => agotados.get(m) ?? 0));
      throw new AppError(
        429,
        'CUOTA_EXCEDIDA',
        'Se agoto la cuota diaria de todos los modelos disponibles.',
        `El nivel gratuito de Gemini permite 20 consultas al dia por modelo. ` +
        ...
        try {
          const respuesta = await ai.models.generateContent({
            ...
            config: {
              ...
              // Razonamiento explicito antes de responder. Discriminar entre especies
          ...
          if (bloqueo) {
            throw new AppError(
              422,
              'CONTENIDO_BLOQUEADO',
              'Gemini bloqueo la consulta por sus filtros de contenido.',
              `Motivo: ${bloqueo}. Prueba con otra fotografia.`
            );
          }
          ...
          return { resultado, modelo, uso: respuesta?.usageMetadata ?? null };
        } catch (error) {
          ...
          if (status === 404) {
            console.warn(`[gemini:${nombre}] ${modelo} no disponible para esta clave, se salta`);
          ...
          if (intento < config.geminiMaxRetries && esSaturacion(error)) {
            ...
            console.warn(
              `[gemini:${nombre}] ${modelo}: intento ${intento + 1} fallido ` +
                `(${status ?? 'sin codigo'}), reintentando en ${espera} ms`
            );
            await dormir(espera);
            continue;
          }
          ...
          if (
            status === 401 ||
            status === 403 ||
            /API_KEY_INVALID|API key not valid/i.test(String(error?.message ?? ''))
          ) {
          ...
          console.warn(
            `[gemini:${nombre}] ${modelo} descartado (${status ?? 'sin codigo'}), se prueba el siguiente`
          );
